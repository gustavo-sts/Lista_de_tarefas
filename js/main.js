...
function filter_tasks(el) {
  ...
  if (window.innerWidth <= 750) {
    ...
    nav.querySelector(".navigation-title-box").innerHTML = '<i class="fa-solid fa-bars"></i>';

    nav.querySelectorAll(".navigation-elements-box span").forEach((el) => (el.style.display = "none"));

    document.querySelector(".navigation-title-box").addEventListener("click", () => {
      if (nav.style.width !== "90%") {
          
          nav.style = "width: 90%; height: 700px;";

          content.style = "position: absolute; z-index: -1; height: 700px; width: 80%; left: 70px";

          nav.querySelectorAll(".navigation-elements-box span").forEach((el) => (el.style.display = "flex"));

          nav.querySelector(".navigation-title-box")
          .innerHTML = '<h2>Navegação</h2> <i class="fa-regular fa-circle-xmark"></i>';
        } else {
